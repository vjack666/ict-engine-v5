...
class PrintMigrationTool:
    ...
    def __init__(self, project_root: Path):
        ...
        # [TARGET] Patrones de print statements a buscar
        self.print_patterns = [
            r'print\s*\(',  # print(
            r'print\s+',    # print (space)
        ]

        # [TOOL] Archivos a excluir de la migración
        self.exclude_files = {
            '__pycache__',
            '.git',
            '.vscode',
        ...
        # [FOLDER] Extensiones de archivo a procesar
        self.target_extensions = {'.py'}

    def scan_project(self) -> Dict:
        """Escanea todo el proyecto buscando print statements"""
        print("[SCAN] Escaneando proyecto en busca de print statements...")
    ...
    def _find_prints_in_file(self, file_path: Path) -> List[Dict]:
        ...
        try:
            ...
            for line_num, line in enumerate(lines, 1):
                # [SCAN] Buscar patrones de print
                for pattern in self.print_patterns:
                    matches = re.finditer(pattern, line)
                    for match in matches:
                        # [OK] Verificar que no sea un comentario
    ...
    def _is_commented_out(self, line: str, position: int) -> bool:
        ...
        # [SCAN] Buscar # antes de la posición
        hash_pos = line.find('#')
    ...
    def migrate_file(self, file_path: Path, dry_run: bool = True) -> Dict:
        ...
        try:
            ...
            # [SAVE] Guardar cambios si no es dry run
            if changes_made and not dry_run:
                ...
                # [SAVE] Escribir archivo modificado
    ...
    def _migrate_line(self, line: str, line_number: int, file_path: Path) -> str:
        ...
        # [SCAN] Buscar y reemplazar print statements
        modified_line = line

        # [TARGET] Patrón más específico para capturar el contenido del print
        print_pattern = r'print\s*\(\s*([^)]+)\s*\)'

        def replace_print(match):
            print_content = match.group(1).strip()

            # [TARGET] Determinar el tipo de log basado en el contenido
            log_type = self._determine_log_type(print_content)

            # [TOOL] Crear llamada a enviar_senal_log
        ...
        # [OK] Verificar si necesitamos añadir import
    ...
    def migrate_all_files(self, dry_run: bool = True) -> Dict:
        ...
        print(f"[LAUNCH] {'[DRY RUN] ' if dry_run else ''}Iniciando migración de print statements...")

        # [SCAN] Primero escanear
        scan_results = self.scan_project()

        if scan_results['total_prints'] == 0:
            print("[OK] No se encontraron print statements para migrar")
            return {
        ...
        print(f"[REPORT] Encontrados {scan_results['total_prints']} prints en {len(scan_results['files_with_prints'])} archivos")
        ...
        for file_info in scan_results['files_with_prints']:
            ...
            if result['prints_migrated'] > 0:
                print(f"  {'[DRY RUN] ' if dry_run else ''}[OK] {file_info['file']}: {result['prints_migrated']} prints migrados")
...
def main():
    ...
    # [TARGET] Inicializar herramienta
    migration_tool = PrintMigrationTool(args.project_root)

    if args.scan_only:
        # [SCAN] Solo escanear
        print("[SCAN] MODO SCAN - Solo escanear print statements")
        results = migration_tool.scan_project()

        print(f"\n[REPORT] RESULTADOS DEL SCAN:")
        print(f"  [FOLDER] Archivos escaneados: {results['files_scanned']}")
        print(f"  [LIST] Archivos con prints: {len(results['files_with_prints'])}")
        print(f"  [TARGET] Total prints encontrados: {results['total_prints']}")

        if results['files_with_prints']:
            print(f"\n[LIST] ARCHIVOS CON PRINT STATEMENTS:")
            for file_info in results['files_with_prints']:
                print(f"  [FILE] {file_info['file']}: {file_info['count']} prints")

    elif args.dry_run:
        # [TEST] Dry run
        print("[TEST] MODO DRY RUN - Mostrar cambios sin aplicar")
        results = migration_tool.migrate_all_files(dry_run=True)

        print(f"\n[REPORT] RESULTADOS DEL DRY RUN:")
        print(f"  [FOLDER] Archivos procesados: {results['files_processed']}")
        print(f"  [TARGET] Prints encontrados: {results['prints_found']}")
        print(f"  🔄 Prints que se migrarían: {results['prints_migrated']}")
        print(f"  [CHART] Estado: {results['status']}")

    else:
        # [LAUNCH] Migración real
        print("[LAUNCH] MODO MIGRACIÓN - Aplicando cambios")
        results = migration_tool.migrate_all_files(dry_run=False)

        print(f"\n[REPORT] RESULTADOS DE LA MIGRACIÓN:")
        print(f"  [FOLDER] Archivos procesados: {results['files_processed']}")
        print(f"  [TARGET] Prints encontrados: {results['prints_found']}")
        print(f"  [OK] Prints migrados: {results['prints_migrated']}")
        print(f"  [CHART] Estado: {results['status']}")

        if results['status'] == 'COMPLETED':
            print("[SUCCESS] ¡Migración completada exitosamente!")
        elif results['status'] == 'PARTIAL':
            print("[WARNING] Migración parcial - revisar errores")

        # 🔐 Recordar sobre backups
        if migration_tool.migration_report['files_modified']:
            print(f"\n[SAVE] BACKUPS CREADOS:")
