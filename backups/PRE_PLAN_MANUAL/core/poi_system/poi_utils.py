...
def calcular_riesgo_poi(poi: Dict, current_price: float, position_size: float = 1.0) -> Dict:
    ...
    Calcula el riesgo asociado con un POI específico basado en:
    ...
    Args:
        ...
        position_size: Tamaño de posición (lots)

    Returns:
        Dict con métricas de riesgo calculadas
    """
    try:
        # Extraer datos del POI con validación de tipos
        poi_price = float(poi.get('price', current_price))
        poi_type = str(poi.get('type', 'UNKNOWN'))

        # Convertir strength si viene como string
        strength_raw = poi.get('strength', 50.0)
        if isinstance(strength_raw, str):
            strength_map = {'BAJA': 25.0, 'MEDIA': 50.0, 'ALTA': 75.0, 'MUY_ALTA': 90.0}
            strength = strength_map.get(strength_raw.upper(), 50.0)
        else:
            strength = float(strength_raw)

        # Convertir confidence si viene como string
        confidence_raw = poi.get('confidence', 0.5)
        if isinstance(confidence_raw, str):
            confidence_map = {'BAJA': 0.25, 'MEDIA': 0.5, 'ALTA': 0.75, 'MUY_ALTA': 0.9}
            confidence = confidence_map.get(confidence_raw.upper(), 0.5)
        else:
            confidence = float(confidence_raw)

        # Calcular distancia en pips (asumiendo EUR/USD como ejemplo)
        pip_value = 0.0001  # Para pares major
        distance_pips = abs(current_price - poi_price) / pip_value

        # Factor de riesgo base según tipo de POI
        risk_factors = {
            'BULLISH_OB': 0.6,    # Order blocks son más confiables
            'BEARISH_OB': 0.6,
            'BULLISH_FVG': 0.8,   # FVG tienen más riesgo
            'BEARISH_FVG': 0.8,
            'BULLISH_BREAKER': 0.7,  # Breakers nivel medio
            'BEARISH_BREAKER': 0.7,
            'LIQUIDITY_VOID': 0.9,   # Alta incertidumbre
            'PRICE_IMBALANCE': 0.75
        }

        base_risk_factor = risk_factors.get(poi_type, 0.8)

        # Ajustar riesgo por distancia (más lejos = más riesgo)
        distance_risk_factor = min(1.0, distance_pips / 100.0)  # Max riesgo a 100 pips

        # Ajustar por fortaleza del POI (más fuerte = menos riesgo)
        strength_risk_factor = max(0.1, (100 - strength) / 100.0)

        # Ajustar por confianza (más confianza = menos riesgo)
        confidence_risk_factor = max(0.1, 1.0 - confidence)

        # Calcular riesgo total
        total_risk_factor = (
            base_risk_factor * 0.4 +
            distance_risk_factor * 0.2 +
            strength_risk_factor * 0.2 +
            confidence_risk_factor * 0.2
        )

        # Calcular riesgo monetario
        pip_cost = position_size * 10  # $10 por pip para 1 lot en EUR/USD
        potential_loss = distance_pips * pip_cost * total_risk_factor

        # Clasificar nivel de riesgo
        if total_risk_factor <= 0.3:
            risk_level = "BAJO"
            risk_emoji = "🟢"
        elif total_risk_factor <= 0.6:
            risk_level = "MEDIO"
            risk_emoji = "🟡"
        else:
            risk_level = "ALTO"
        ...
        if poi_type.startswith('BULLISH'):
            suggested_sl = poi_price - (distance_pips * 0.2 * pip_value)  # 20% debajo del POI
        else:
            suggested_sl = poi_price + (distance_pips * 0.2 * pip_value)  # 20% arriba del POI
        ...
        risk_metrics = {
            ...
            'suggested_stop_loss': round(suggested_sl, 5),
            'pip_cost': pip_cost,
            'components': {
                'base_risk': round(base_risk_factor, 3),
                'distance_risk': round(distance_risk_factor, 3),
                'strength_risk': round(strength_risk_factor, 3),
                'confidence_risk': round(confidence_risk_factor, 3)
            },
            'poi_data': {
                'type': poi_type,
                'price': poi_price,
        ...
        enviar_senal_log(
            "INFO",
            f"🎯 Riesgo POI calculado: {risk_emoji} {risk_level} ({total_risk_factor:.1%}) - ${potential_loss:.2f} loss potential",
            __name__,
            "poi_risk"
        )
        ...
        enviar_senal_log("ERROR", f"❌ Error calculando riesgo POI: {e}", __name__, "poi_risk")

        # Retornar métricas de error
        return {
            'total_risk_factor': 1.0,  # Máximo riesgo por error
            'risk_level': "ERROR",
            'risk_emoji': "⚠️",
            'distance_pips': 0,
            ...
            'error': str(e),
            'poi_data': poi,
            'market_data': {'current_price': current_price, 'position_size_lots': position_size}
        }
...
if __name__ == "__main__":
    ...
    current_price = 1.16200
    risk_metrics = calcular_riesgo_poi(test_poi, current_price, 1.0)
    enviar_senal_log("INFO", f"🎯 Test riesgo: {risk_metrics['risk_level']} - ${risk_metrics['potential_loss_usd']}", __name__, "poi")
