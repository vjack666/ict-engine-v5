...
class MarketContextV6:
    ...
    def __init__(self, memory_config_path: str = "config/memory_config.json"):
        ...
        # === SWING POINTS HISTÓRICOS ===
        self.swing_points: Dict[str, List[dict]] = {
            "highs": [],
            "lows": [],
            "last_high": None,
            "last_low": None
        ...
        self.daily_range: Dict[str, float] = {'high': 0, 'low': 0, 'mid': 0}
        self.trading_conditions: dict = {}
    ...
    def update_market_context(self, analysis_results: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Actualizar bias si está disponible
            if 'market_bias' in analysis_results:
                old_bias = self.market_bias
                self.market_bias = analysis_results['market_bias']
            ...
            # Actualizar bias multi-timeframe
            if 'timeframe_bias' in analysis_results:
                for tf, bias in analysis_results['timeframe_bias'].items():
                    if tf in self.timeframe_bias:
                        self.timeframe_bias[tf] = bias
            
            # Actualizar contexto multi-timeframe
            if 'timeframe_results' in analysis_results:
                for tf, tf_data in analysis_results['timeframe_results'].items():
                    if tf in self.timeframe_contexts:
                        self.timeframe_contexts[tf].update({
                            'last_analysis': tf_data,
                            'last_updated': self.last_updated
                        })
            
            # Actualizar memoria de patrones ICT
            self._update_pattern_memory(analysis_results)
            
            # Actualizar memoria Smart Money
    ...
    def _update_pattern_memory(self, analysis_results: Dict[str, Any]) -> None:
        ...
        try:
            ...
            # Actualizar swing points
            if 'swing_points' in analysis_results:
                swing_data = analysis_results['swing_points']
                if 'high' in swing_data:
                    self.swing_points['highs'].append({
                        'timestamp': self.last_updated,
                        'price': swing_data['high'],
                        'data': swing_data
                    })
                    self.swing_points['last_high'] = swing_data['high']
                
                if 'low' in swing_data:
                    self.swing_points['lows'].append({
                        'timestamp': self.last_updated,
                        'price': swing_data['low'],
                        'data': swing_data
                    })
                    self.swing_points['last_low'] = swing_data['low']
                
                # Mantener límite de swing points
                if len(self.swing_points['highs']) > self.max_swing_points:
                    self.swing_points['highs'] = self.swing_points['highs'][-self.max_swing_points:]
                if len(self.swing_points['lows']) > self.max_swing_points:
                    self.swing_points['lows'] = self.swing_points['lows'][-self.max_swing_points:]
    ...
    def get_historical_context(self, lookback_periods: int = None) -> Dict[str, Any]:
        """
    ...
    def _get_swing_context(self, periods: int) -> Dict[str, Any]:
        """Obtiene contexto de swing points."""
        return {
            'recent_highs': self.swing_points['highs'][-periods:],
            'recent_lows': self.swing_points['lows'][-periods:],
            'last_high': self.swing_points['last_high'],
            'last_low': self.swing_points['last_low']
        }
    ...
    def persist_memory_state(self) -> None:
        ...
        Como un trader real que registra su journal.
        """
        try:
            memory_state = {
                ...
                'pattern_memory': {
                    ...
                    'swing_points': {
                        'recent_highs': self.swing_points['highs'][-20:],
                        'recent_lows': self.swing_points['lows'][-20:],
                        'last_high': self.swing_points['last_high'],
                        'last_low': self.swing_points['last_low']
                    }
                },
                'smart_money_memory': self.smart_money_context,
                'killzone_memory': self.killzone_memory
            }
            ...
            self.logger.info(f"💾 Estado de memoria persistido en {memory_file}", 
                               component="market_memory")
    ...
    def restore_memory_state(self) -> bool:
        ...
        Como un trader real consultando su journal histórico.
        """
        try:
            memory_file = os.path.join(self.memory_cache_dir, 'market_context_state.json')
            
            if not os.path.exists(memory_file):
                self.logger.warning("No se encontró estado de memoria previo", 
                                       component="market_memory")
                return False
            ...
            if 'pattern_memory' in memory_state:
                ...
                if 'swing_points' in pm:
                    sp = pm['swing_points']
                    self.swing_points['highs'] = sp.get('recent_highs', [])
                    self.swing_points['lows'] = sp.get('recent_lows', [])
                    self.swing_points['last_high'] = sp.get('last_high')
                    self.swing_points['last_low'] = sp.get('last_low')
            
            # Restaurar memoria Smart Money
            if 'smart_money_memory' in memory_state:
                self.smart_money_context = memory_state['smart_money_memory']
            
            if 'killzone_memory' in memory_state:
                self.killzone_memory = memory_state['killzone_memory']
            
            self.logger.info(f"📚 Estado de memoria restaurado desde {memory_file}", 
                               component="market_memory")
            return True
