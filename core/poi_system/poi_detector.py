#!/usr/bin/env python3
"""
🎯 POI DETECTOR - Sistema Consolidado de Detección de Puntos de Interés
========================================================================

Módulo unificado que consolida toda la funcionalidad de detección, análisis y scoring de POIs.

Funcionalidades principales:
- Detección de Order Blocks, Fair Value Gaps, Breaker Blocks
- Sistema de scoring avanzado
- Análisis contextual de mercado
- Reportes y estadísticas

Versión: v3.3.3 (Consolidado)
Consolidado desde: poi_system.py, poi_multi_detector.py, poi_system_optimized.py
"""
...
POI_TYPES = {
    ...
    'LIQUIDITY_VOID': 'Vacío de Liquidez',
...
POI_SCORING_CONFIG = {
    ...
    'SCORING_WEIGHTS': {
        ...
        'strength': 20,     # Fuerza de formación
        'timeframe': 15,    # Relevancia del timeframe
        'volume': 10,       # Volumen de formación
        'confirmation': 15, # Confirmación técnica
...
def log_poi_to_csv(component: str, message: str, is_error: bool = False):
    """Función de logging compatible con el sistema existente"""
    try:
        from utils.logging_utils import save_adaptive_debug_to_csv
        save_adaptive_debug_to_csv(f"[{component}] {message}")
    except:
        # Fallback silencioso
        pass
...
def crear_poi_estructura(poi_type: str, price: float, score: int, confidence: float, 
    ...
    """
    Crea estructura estándar para un POI.
    
    Args:
        poi_type: Tipo de POI (usar POI_TYPES)
        price: Precio central del POI
        score: Puntuación del POI (0-100)
        confidence: Nivel de confianza (0.0-1.0)
        timeframe: Timeframe de formación
        **kwargs: Parámetros adicionales específicos del POI
        
    Returns:
        Dict con estructura estándar del POI
    """
    return {
...
def detectar_order_blocks(df: pd.DataFrame, timeframe: str = "M15") -> List[Dict]:
    """
    Detecta Order Blocks (zonas de órdenes institucionales).
    
    Un Order Block es una zona donde las instituciones han colocado órdenes
    masivas, identificada por patrones específicos de velas y volumen.
    ...
    """
    enviar_senal_log("INFO", f"🔍 INICIANDO detección de Order Blocks en timeframe {timeframe}", __name__, "general")
    ...
    try:
        if len(df) < 20:
            enviar_senal_log("WARNING", f"⚠️ Dataset insuficiente para análisis OB: solo {len(df)} velas (mínimo 20)", __name__, "general")
            return order_blocks
        ...
        enviar_senal_log("INFO", f"🎯 DETECCIÓN OB COMPLETADA: {len(order_blocks)} total ({bullish_count} alcistas, {bearish_count} bajistas) en {timeframe}", __name__, "general")
        log_poi_to_csv("OB_DETECTION", f"Detectados {len(order_blocks)} Order Blocks en {timeframe}")
        
    except (ValueError, KeyError, TypeError) as e:
        enviar_senal_log("ERROR", f"❌ ERROR en detección de Order Blocks: {e}", __name__, "general")
        log_poi_to_csv("OB_DETECTION_ERROR", f"Error detectando OBs: {e}", is_error=True)
        
    return order_blocks

def detectar_fair_value_gaps(df: pd.DataFrame, timeframe: str = "M15") -> List[Dict]:
    """
    Detecta Fair Value Gaps (vacíos de precio ineficientes).
    
    Un FVG ocurre cuando hay un gap entre velas que indica ineficiencia
    de precio que el mercado buscará llenar posteriormente.
    ...
    """
    enviar_senal_log("INFO", f"🔍 INICIANDO detección de Fair Value Gaps en timeframe {timeframe}", __name__, "general")
    enviar_senal_log("DEBUG", f"Dataset recibido: {len(df)} velas para análisis FVG", __name__, "general")
    
    fvgs = []
    
    try:
        if len(df) < 3:
            enviar_senal_log("WARNING", f"⚠️ Dataset insuficiente para análisis FVG: solo {len(df)} velas (mínimo 3)", __name__, "general")
            return fvgs
        ...
        enviar_senal_log("INFO", f"🎯 DETECCIÓN FVG COMPLETADA: {len(fvgs)} total ({bullish_fvg_count} alcistas, {bearish_fvg_count} bajistas) en {timeframe}", __name__, "general")
        log_poi_to_csv("FVG_DETECTION", f"Detectados {len(fvgs)} FVGs en {timeframe}")
        
    except (ValueError, KeyError, TypeError) as e:
        enviar_senal_log("ERROR", f"❌ ERROR en detección de Fair Value Gaps: {e}", __name__, "general")
        log_poi_to_csv("FVG_DETECTION_ERROR", f"Error detectando FVGs: {e}", is_error=True)
        
    return fvgs

def detectar_breaker_blocks(df: pd.DataFrame, timeframe: str = "M15") -> List[Dict]:
    """
    ...
    """
    enviar_senal_log("INFO", f"🔍 INICIANDO detección de Breaker Blocks en timeframe {timeframe}", __name__, "general")
    
    breaker_blocks = []
    
    try:
        # Primero detectar Order Blocks
        enviar_senal_log("DEBUG", "Detectando Order Blocks base para análisis de Breakers...", __name__, "general")
        order_blocks = detectar_order_blocks(df, timeframe)
        
        if not order_blocks:
            enviar_senal_log("INFO", "ℹ️ No hay Order Blocks base para detectar Breakers", __name__, "general")
            return breaker_blocks
        
        enviar_senal_log("DEBUG", f"Analizando {len(order_blocks)} Order Blocks para conversión a Breakers...", __name__, "general")
        
        # Buscar OBs que han sido rotos y actúan como breakers
        breaker_count = 0
        ...
        enviar_senal_log("INFO", f"🎯 DETECCIÓN BREAKER COMPLETADA: {len(breaker_blocks)} Breaker Blocks de {len(order_blocks)} OBs analizados en {timeframe}", __name__, "general")
        log_poi_to_csv("BREAKER_DETECTION", f"Detectados {len(breaker_blocks)} Breaker Blocks en {timeframe}")
        
    except (ValueError, KeyError, TypeError) as e:
        enviar_senal_log("ERROR", f"❌ ERROR en detección de Breaker Blocks: {e}", __name__, "general")
        log_poi_to_csv("BREAKER_DETECTION_ERROR", f"Error detectando Breakers: {e}", is_error=True)
        
    return breaker_blocks

def detectar_imbalances(df: pd.DataFrame, timeframe: str = "M15") -> List[Dict]:
    """
    ...
    """
    enviar_senal_log("INFO", f"🔍 INICIANDO detección de Imbalances (vacíos de liquidez", __name__, "general")
    enviar_senal_log("DEBUG", f"Dataset recibido: {len(df)} velas para análisis de imbalances", __name__, "general")
    
    imbalances = []
    
    try:
        if len(df) < 10:
            enviar_senal_log("WARNING", f"⚠️ Dataset insuficiente para análisis de imbalances: solo {len(df)} velas (mínimo 10)", __name__, "general")
            return imbalances
        ...
        # Buscar vacíos de liquidez (movimientos rápidos con poco volumen)
        for i in range(5, len(df) - 5):
            current = df.iloc[i]
            prev_window = df.iloc[i-5:i]
            next_window = df.iloc[i+1:i+6]
            
            # Detectar movimiento rápido con bajo volumen
        ...
        enviar_senal_log("INFO", f"🎯 DETECCIÓN IMBALANCES COMPLETADA: {len(imbalances)} imbalances detectados en {timeframe}", __name__, "general")
        enviar_senal_log("DEBUG", f"   Movimientos de bajo volumen detectados: {low_volume_moves}", __name__, "general")
        log_poi_to_csv("IMBALANCE_DETECTION", f"Detectados {len(imbalances)} imbalances en {timeframe}")
        
    except (ValueError, KeyError, TypeError) as e:
        enviar_senal_log("ERROR", f"❌ ERROR en detección de imbalances: {e}", __name__, "general")
        log_poi_to_csv("IMBALANCE_DETECTION_ERROR", f"Error detectando imbalances: {e}", is_error=True)
        
    return imbalances

# =============================================================================
# FUNCIÓN PRINCIPAL DE ORQUESTACIÓN POI
# =============================================================================

def detectar_todos_los_pois(df: pd.DataFrame, timeframe: str = "M15", 
                           current_price: Optional[float] = None) -> Dict[str, List[Dict]]:
    """
    FUNCIÓN PRINCIPAL: Detecta todos los tipos de POIs en un DataFrame.
    
    Esta función orquesta todas las sub-rutinas de detección de POIs
    y proporciona logging granular del proceso completo.
    
    Args:
        df: DataFrame con datos OHLC
        timeframe: Timeframe de análisis
        current_price: Precio actual para cálculos de proximidad
    ...
    try:
        ...
        enviar_senal_log("INFO", f"🎯 DETECCIÓN COMPLETA FINALIZADA", __name__, "general")
        ...
        enviar_senal_log("INFO", f"📊 DISTRIBUCIÓN: OB={len(obs)}, FVG={len(fvgs)}, BB={len(breakers)}, IM={len(imbalances)}", __name__, "general")
        
        # Log a CSV para seguimiento
        log_poi_to_csv("DETECCION_COMPLETA", 
        ...
        enviar_senal_log("ERROR", f"❌ ERROR CRÍTICO en detección completa de POIs: {e}", __name__, "general")
        log_poi_to_csv("DETECCION_COMPLETA_ERROR", f"Error crítico en TF {timeframe}: {e}", is_error=True)
        return todos_los_pois

# =============================================================================
# FUNCIONES DE SCORING Y ANÁLISIS
# =============================================================================

def calcular_puntaje_poi(poi: Dict, mercado: Any, config: Dict, 
                        df_m15: Optional[pd.DataFrame] = None) -> Tuple[int, List[str]]:
    """
    Calcula el puntaje de un POI basado en múltiples factores.
    
    Args:
        poi: Diccionario del POI a evaluar
        mercado: Contexto de mercado
        config: Configuración de scoring
        df_m15: DataFrame M15 para análisis adicional
    ...
    try:
        ...
        # 1. Proximidad al precio actual (25 puntos máximo)
        ...
        # 2. Fuerza de formación (20 puntos máximo)
        ...
        # 3. Relevancia del timeframe (15 puntos máximo)
        ...
        # 5. Confirmación técnica (15 puntos máximo)
        ...
        detalles.append(f"Confirmación: {confirmacion_score:.1f}%")
        
        # 6. Frescura (10 puntos máximo)
        ...
        # 7. Contexto de mercado (5 puntos máximo)
        ...
        log_poi_to_csv("SCORING_ERROR", f"Error calculando score POI: {e}", is_error=True)
        return 0, ["Error en cálculo"]

def obtener_pois_superiores_cercanos(mercado: Any, current_price: float, 
                                    config: Optional[Dict] = None) -> Optional[Dict]:
    """
    Identifica los POIs de mayor calidad más cercanos al precio actual.
    ...
    Returns:
        Dict con información de POIs superiores o None
    """
    try:
        if config is None:
        ...
        log_poi_to_csv("SUPERIOR_POIS_ERROR", f"Error obteniendo POIs superiores: {e}", is_error=True)
        return None

def encontrar_poi_de_alta_probabilidad(mercado: Any, current_price: float, 
                                     config: Optional[Dict] = None,
                                     df_m15: Optional[pd.DataFrame] = None) -> Optional[Dict]:
    """
    Encuentra el POI de más alta probabilidad para trading.
    ...
    try:
            ...
            if isinstance(pois, list):
                for poi in pois:
                    if not poi.get('broken', False) and not poi.get('mitigated', False):
                        ...
                        # Solo considerar POIs con score mínimo
        ...
        log_poi_to_csv("HIGH_PROB_POI_ERROR", f"Error encontrando POI alta probabilidad: {e}", is_error=True)
        return None

def encontrar_pois_multiples_para_dashboard(mercado: Any, current_price: float,
                                          max_pois: int = 5) -> List[Dict]:
    """
    Encuentra múltiples POIs para mostrar en el dashboard.
    
    Args:
        mercado: Contexto de mercado
        current_price: Precio actual
        max_pois: Número máximo de POIs a retornar
        
    Returns:
        Lista de POIs ordenados por relevancia
    """
    enviar_senal_log("INFO", f"🎯 BÚSQUEDA DE POIs PARA DASHBOARD iniciada | Precio actual: {current_price:.5f} | Max POIs: {max_pois}", __name__, "general")
    
    try:
        if not mercado or not hasattr(mercado, 'pois'):
            enviar_senal_log("WARNING", "⚠️ No hay contexto de mercado o POIs disponibles", __name__, "general")
            return []
        ...
        if not todos_los_pois:
            enviar_senal_log("WARNING", "⚠️ No se encontraron POIs activos en ningún timeframe", __name__, "general")
            return []
        ...
        enviar_senal_log("ERROR", f"❌ ERROR en búsqueda de POIs para dashboard: {e}", __name__, "general")
        log_poi_to_csv("DASHBOARD_POIS_ERROR", f"Error preparando POIs para dashboard: {e}", is_error=True)
        return []
...
def invalidar_pois_obsoletos(mercado: Any, df_h4: pd.DataFrame) -> int:
    """
            ...
            if isinstance(pois, list):
                for poi in pois:
                    ...
                    if _verificar_ruptura_poi(poi, current_price, df_h4):
                        ...
                        invalidados += 1
                        log_poi_to_csv("POI_INVALIDATION", f"POI {poi['id']} invalidado por ruptura")
        
        return invalidados
        
    except (ValueError, KeyError, TypeError) as e:
        log_poi_to_csv("INVALIDATION_ERROR", f"Error invalidando POIs: {e}", is_error=True)
        return 0
...
def generar_mensaje_contextual_pois(info_contextual: Dict, current_price: float) -> str:
    ...
    Args:
        info_contextual: Información contextual de POIs
        current_price: Precio actual
    ...
    try:
        ...
        log_poi_to_csv("CONTEXTUAL_MSG_ERROR", f"Error generando mensaje: {e}", is_error=True)
        return "Error generando información contextual de POIs."
...
def generar_reporte_pois(mercado: Any) -> str:
    ...
    try:
        if not mercado or not hasattr(mercado, 'pois'):
            return "No hay información de POIs disponible."
        ...
        log_poi_to_csv("REPORT_ERROR", f"Error generando reporte: {e}", is_error=True)
...
class POIDetector:
    ...
    Clase encapsuladora que actúa como la interfaz pública profesional
    para todo el sistema de detección de POIs. Proporciona un punto
    ...
    def find_all_pois(self, df: pd.DataFrame, timeframe: str, 
                     current_price: Optional[float] = None) -> List[Dict]:
        """
        🎯 PUNTO DE ENTRADA PRINCIPAL PARA EL ACC
        ========================================
        
        Orquesta todas las funciones de detección individuales y
        consolida TODOS los POIs en una lista unificada.
        
        Args:
            df: DataFrame con datos OHLC
            timeframe: Timeframe de análisis
            current_price: Precio actual para cálculos
        ...
        try:
            ...
            if df.empty:
                enviar_senal_log("WARNING", "⚠️ DataFrame vacío recibido", __name__, "detection")
                return []
            
            detection_start = datetime.now()
            all_pois = []
            
            # 🔍 ORQUESTAR TODAS LAS FUNCIONES DE DETECCIÓN
            enviar_senal_log("DEBUG", "Iniciando detección Order Blocks...", __name__, "detection")
            order_blocks = detectar_order_blocks(df, timeframe)
            all_pois.extend(order_blocks)
            
            enviar_senal_log("DEBUG", "Iniciando detección Fair Value Gaps...", __name__, "detection")
            fair_value_gaps = detectar_fair_value_gaps(df, timeframe)
            all_pois.extend(fair_value_gaps)
            
            enviar_senal_log("DEBUG", "Iniciando detección Breaker Blocks...", __name__, "detection")
            breaker_blocks = detectar_breaker_blocks(df, timeframe)
            all_pois.extend(breaker_blocks)
            
            enviar_senal_log("DEBUG", "Iniciando detección Imbalances...", __name__, "detection")
            imbalances = detectar_imbalances(df, timeframe)
    ...
    def find_dashboard_pois(self, df: pd.DataFrame, timeframe: str, 
                           current_price: float, max_pois: int = 5) -> List[Dict]:
        """
        🎯 ESPECIALIZACIÓN PARA DASHBOARD
        ================================
        
        Encuentra POIs optimizados para visualización en dashboard.
        
        Args:
            df: DataFrame con datos OHLC
            timeframe: Timeframe de análisis
            current_price: Precio actual
            max_pois: Máximo número de POIs a retornar
        ...
        try:
            ...
            # Usar la función existente optimizada para dashboard
            class MockMercado:
            ...
            # Crear mock mercado para usar función existente
    ...
    def get_poi_summary(self, pois: List[Dict]) -> Dict:
        """
        📊 Generar resumen estadístico de POIs
        
        Args:
            pois: Lista de POIs
            
        Returns:
            Dict con estadísticas de POIs
        """
        try:
            if not pois:
...
# =============================================================================
# FUNCIONES DE TESTING Y CONFIGURACIÓN
# =============================================================================
...
def set_poi_debug_mode(enabled: bool):
    ...
    """
    # Logging solo - sin modificación de estado global
    log_poi_to_csv("DEBUG_MODE", f"Modo debug POI: {'ACTIVADO' if enabled else 'DESACTIVADO'}")
...
def _is_bullish_order_block(current, prev, next_candles):
    ...
    return (current['close'] > current['open'] and  # Vela alcista
            ...
            next_candles['close'].max() > current['high'] * 1.001)  # Confirmación
...
def _is_bearish_order_block(current, prev, next_candles):
    ...
    return (current['close'] < current['open'] and  # Vela bajista
            ...
            next_candles['close'].min() < current['low'] * 0.999)  # Confirmación

def _calcular_score_ob(current, next_candles, direction):
    """Calcula score específico para Order Blocks"""
    base_score = 60
...
def _determinar_confianza_ob(current, next_candles):
    """Determina confianza del Order Block (sin volumen si no está disponible)"""
    try:
        # Intentar usar volumen si está disponible
        if 'volume' in current.index and 'volume' in next_candles.columns:
            ...
            # Fallback: usar price action para confirmar (más de 3 velas siguientes)
...
def _calcular_score_fvg(prev_candle, current_candle, next_candle, direction):
    """Calcula score específico para Fair Value Gaps"""
    base_score = 55
...
def _is_breaker_block(ob, df):
    ...
    # Lógica simplificada: si el precio ha roto y retestado el OB
    ob_level = ob['price']
...
def _calcular_score_frescura(created_at):
    """Calcula score basado en qué tan reciente es el POI"""
    try:
...
def _calcular_score_contexto(poi, mercado):
    ...
    # Lógica simplificada de contexto
    context_score = 50
    
    # Bonus si el POI está alineado con el bias del mercado
...
def _generar_recomendacion_pois(top_pois, current_price):
    """Genera recomendación basada en los mejores POIs"""
    if not top_pois:
        return "No hay POIs relevantes para recomendación."
...
def _generar_recomendacion_trading(poi, mercado):
    """Genera recomendación específica de trading"""
    ...
    return {
        ...
        'stop_loss': stop_loss,
        'risk_pips': risk_pips,
        'recommended_lot_size': f"Ajustar según {risk_pips:.1f} pips de riesgo"
    }

def _evaluar_riesgo_poi(poi, mercado):
    """Evalúa el riesgo asociado con el POI"""
    risk_level = "MEDIO"
    risk_factors = []
...
def _verificar_ruptura_poi(poi, current_price, df_h4):
    ...
    try:
        # Lógica de ruptura basada en el tipo de POI
...
# =============================================================================
# EXPORTACIONES PÚBLICAS - OPTIMIZADAS PARA ACC
# =============================================================================
