        ...
        try:
            ...
            if session_info and 'session' in session_info:
                ...
                session_activa = {
                    ...
                    'all_active': [session_name]
                }

            # ⏰ PRÓXIMA SESIÓN
            proxima_sesion = {
                'time_string': session_info.get('time_to_next', 'Calculando...') if session_info else 'Calculando...',
            ...
            status_completo = {
                ...
                'status_display': f"{market_status['emoji']} {market_status['status']}",  # ✅ AGREGADO: Formato completo para dashboard
                'is_weekend': now_utc.weekday() >= 5,  # ✅ AGREGADO: Detectar fin de semana (Sábado=5, Domingo=6)
            ...
            return {
                ...
                'market_status': 'ERROR',
                'emoji_status': '❌',
                'status_display': '❌ ERROR',  # ✅ AGREGADO: Formato completo para dashboard
                'is_weekend': datetime.now(timezone.utc).weekday() >= 5,  # ✅ AGREGADO: Detectar fin de semana incluso en error
