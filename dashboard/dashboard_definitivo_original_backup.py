...
class SentinelDashboardDefinitivo(App):
        ...
        try:
            ...
            for data, tf_name in timeframes_data:
                if data is not None and not data.empty:
                    ...
                    if self.debug_mode:
                        enviar_senal_log("INFO", f"🎯 POIs {tf_name}: {len(pois_tf) if isinstance(pois_tf, (list, dict)) else 0} detectados", "dashboard_definitivo", "migration")
    ...
    def sync_poi_logs_with_health_analyzer(self, scored_pois: List[Dict]):
        ...
        try:
            ...
            enviar_senal_log("INFO", f"📊 Sincronizado con Health Analyzer: {len(scored_pois)} POIs registrados en poi_detection.log", "dashboard_definitivo", "poi_sync")

        except (FileNotFoundError, PermissionError, IOError) as e:
            enviar_senal_log("ERROR", f"❌ Error sincronizando logs POI: {e}", "dashboard_definitivo", "poi_sync")
    ...
    def update_from_controller(self, controller_state):
        ...
        try:
            ...
            if controller_state:
                ...
                enviar_senal_log("INFO", "✅ [DASHBOARD-CALLBACK] Estado del controller procesado exitosamente", "dashboard_definitivo", "callback")
            else:
                enviar_senal_log("WARNING", "⚠️ [DASHBOARD-CALLBACK] Estado del controller vacío o None", "dashboard_definitivo", "callback")

        except (FileNotFoundError, PermissionError, IOError) as e:
            enviar_senal_log("ERROR", f"❌ [DASHBOARD-CALLBACK] Error procesando estado del controller: {e}", "dashboard_definitivo", "callback")
            enviar_senal_log("ERROR", f"❌ [DASHBOARD-CALLBACK] Traceback: {traceback.format_exc()}", "dashboard_definitivo", "callback")
